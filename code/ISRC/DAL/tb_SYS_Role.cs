...
* Ver    变更日期             负责人  变更内容
* ───────────────────────────────────
* V0.01  2016/2/25 14:00:50   N/A    初版
*
...
namespace ISRC.DAL
{
	...
	public partial class tb_SYS_Role
	{
		...
		public bool Exists(string ID)
		{
			...
			strSql.Append("select count(1) from tb_SYS_Role");
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(ISRC.Model.tb_SYS_Role model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into tb_SYS_Role(");
			strSql.Append("ID,RoleNO,RoleName,ParentID,DefaultUrl,State,CreateID,CreateTime,UpdateID,UpdateTime)");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(ISRC.Model.tb_SYS_Role model)
		{
			StringBuilder strSql=new StringBuilder();
			...
			parameters[3].Value = model.DefaultUrl;
			parameters[4].Value = model.State;
			parameters[5].Value = model.CreateID;
			parameters[6].Value = model.CreateTime;
			parameters[7].Value = model.UpdateID;
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from tb_SYS_Role ");
        ...
        public string Delete(string id)
        {
            ...
            DbHelperSQL.RunProcedure("proc_delete_Role", parameters).Close();
		...
		/// 得到一个对象实体
		/// </summary>
		public ISRC.Model.tb_SYS_Role GetModel(string ID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,RoleNO,RoleName,ParentID,DefaultUrl,State,CreateID,CreateTime,UpdateID,UpdateTime from tb_SYS_Role ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.NVarChar,64)			};
			parameters[0].Value = ID;

			ISRC.Model.tb_SYS_Role model=new ISRC.Model.tb_SYS_Role();
		...
		/// 得到一个对象实体
		/// </summary>
		public ISRC.Model.tb_SYS_Role DataRowToModel(DataRow row)
		{
			ISRC.Model.tb_SYS_Role model=new ISRC.Model.tb_SYS_Role();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,RoleNO,RoleName,ParentID,DefaultUrl,State,CreateID,CreateTime,UpdateID,UpdateTime ");
        ...
        public DataSet GetListByPage(string sqlWhere, string order, int pageSize, int pageIndex, out long totalRecord)
        {
            ...
            DataSet ds = pageData.getDataPage("tb_SYS_Role", "*", sqlWhere, order, pageSize, pageIndex, out totalRecord);
            return ds;
        }
